    ...
    def generate_image(self, request: ImageGenerationRequest) -> ImageGenerationResult:
        ...
        try:
            ...
            if request.negative_prompt:
                combined_prompt = f"{combined_prompt}. Avoid: {request.negative_prompt}"
            
            # Prepare input data for Replicate (FLUX Depth expects `control_image`)
            input_data = {
                "control_image": image_file,
            ...
            if output:
                if isinstance(output, list):
                    for item in output:
                        if isinstance(item, str):
                            ...
                            for k in ("images", "image", "output", "url"):
                                v = item.get(k)
                                if isinstance(v, str):
                ...
                if generated_images:
                    ...
                    if unique_urls:
                        return ImageGenerationResult(
                            success=True,
                            generated_image_urls=unique_urls
                        )
                return ImageGenerationResult(
                    success=False,
